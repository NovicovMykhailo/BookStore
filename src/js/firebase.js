...
export class useFirebase {
  ...
  findUserAndDatabaseIdToLocalStorage(userIdInAuth) {
    ...
    const userWithId = query(
      collection(db, 'users'),
      where('userIdNum', '==', userIdInAuth)
    );
  ...
  async writeBookArrayToDB(bookObj) {
    const userIdInBase = JSON.parse(
      localStorage.getItem('userBooksIdToCategory')
    );
  ...
  selectBookFromArray(bookId) {
    const q = query(
      ...
      where(
        'userIdNum',
        '==',
        JSON.parse(localStorage.getItem('userIdToLogin'))
      )
    );
